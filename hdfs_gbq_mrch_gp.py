...
if __name__ == "__main__":  
    ...
    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = f'{initial_path}airflow/dags/credentials/dwh-gbq.json'
    ...
    credentials = service_account.Credentials.from_service_account_file(f'{initial_path}airflow/dags/credentials/cred_gbq.json')
    project_id = 'project'
    clientBQ = bigquery.Client(credentials=credentials,project=project_id)

    #ПОЛУЧАЕМ И ПРОВЕРЯЕМ БИЛЕТ KERBEROS
    cred = pd.read_json(f'{initial_path}airflow/dags/credentials/cred.json', typ='series') 
    ...
    req_gbq='''
    SELECT MAX(mt_update_dt) FROM `goods-161014.dwh_input.Dimmrch` 
    '''
    ...
    spark = SparkSession.builder \
        ...
        .config("spark.hadoop.fs.gs.auth.service.account.json.keyfile", f"{initial_path}airflow/dags/credentials/dwh-gbq.json") \
        .config("spark.executorEnv.GOOGLE_APPLICATION_CREDENTIALS", f"{initial_path}airflow/dags/credentials/dwh-gbq.json") \
...
FROM (
  ...
  FROM goods-161014.dwh_input.Dimmrch
